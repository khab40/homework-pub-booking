...
What's different from a pure scaffold:
  ...
  * `example_sessions_dir()` keeps offline and real runs discoverable
...
async def run_scenario(real: bool) -> int:
    ...
    # Ex5 logs are part of the homework evidence, so keep deterministic and
    # real runs discoverable by `make logs` and `make narrate-latest`.
